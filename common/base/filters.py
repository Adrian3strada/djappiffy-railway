...
from .models import CountryProductKindStandard, CountryProductStandardSize, CapitalFramework
...
class ByCountryForMarketProductSizeStandardFilter(admin.SimpleListFilter):
    ...
    def lookups(self, request, model_admin):
        countries_for_market_product_size_standards = list(CountryProductKindStandard.objects.filter(is_enabled=True).values_list('country_id', flat=True).distinct())
