document.addEventListener('DOMContentLoaded', function () {
    function recalculatePercentage() {
        const sampleFields = document.querySelectorAll('input[name$="sample_physical_damage"]');
        const percentageFields = Array.from(document.querySelectorAll('input[name*="samplecollection_set-0-samplephysicaldamage_set-"]'))
        .filter(input => {
            return input.name.endsWith('-percentage') && /^samplecollection_set-\d+-samplephysicaldamage_set-\d+-percentage$/.test(input.name);
        });
