document.addEventListener('DOMContentLoaded', function () {
    ...
    function recalculatePercentage() {
        const sampleFields = document.querySelectorAll('input[name$="sample_disease"]');
        const percentageFields = Array.from(document.querySelectorAll('input[name*="samplecollection_set-0-sampledisease_set-"]'))
        .filter(input => {
            return input.name.endsWith('-percentage') && /^samplecollection_set-\d+-sampledisease_set-\d+-percentage$/.test(input.name);
        });
    ...
    document.addEventListener('input', function (event) {
        if (event.target.matches('input[name$="sample_disease"]') || event.target.matches('input[name$="-weight"]')) {
