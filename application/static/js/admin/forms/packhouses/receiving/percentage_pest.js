document.addEventListener('DOMContentLoaded', function () {
    ...
    function recalculatePercentage() {
        const sampleFields = document.querySelectorAll('input[name$="sample_pest"]');
        const percentageFields = Array.from(document.querySelectorAll('input[name*="samplecollection_set-0-samplepest_set-"]'))
        .filter(input => {
            return input.name.endsWith('-percentage') && /^samplecollection_set-\d+-samplepest_set-\d+-percentage$/.test(input.name);
        });
    ...
    document.addEventListener('input', function (event) {
        if (event.target.matches('input[name$="sample_pest"]') || event.target.matches('input[name$="-weight"]')) {
